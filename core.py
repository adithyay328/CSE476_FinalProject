...
UPDATE: I scrapped this. The Qwen
style model tht the class uses doesn't
support tool calls, which nukes suport
for this entire design. This requires
that models are equipped with more advanced
CFG style parsers, which were made popular with
gpt 3.5 turbo. So, switching to a simpler
design where the agent just loops and
uses a dumber tool call format.
"""
